...
function Clients() {
  ...
  const attendanceSteps = [
    [FileText, 'Diagnostico', 'Levantamento da necessidade, urgencia, processo e condicoes de operacao.'],
    [Settings, 'Proposta tecnica', 'Definicao de escopo, materiais, prazo, documentacao e caminho de execucao.'],
    [Factory, 'Fabricacao e campo', 'Equipe integrada para fabricar, reformar, montar ou acompanhar a parada.'],
    [Gauge, 'Acompanhamento', 'Suporte apos a entrega para reduzir risco e manter performance operacional.']
  ];
  return <><PageTitle eyebrow="Clientes" title="Experiencia com mais de 100 usinas" text="A IMEC construiu sua trajetoria atendendo usinas de acucar e alcool e empresas industriais em projetos, equipamentos e servicos especializados." /><section className="clients-premium"><div className="clients-intro"><span>Confianca em campo</span><h2>Relacionamento tecnico do primeiro contato a operacao</h2><p>O diferencial da IMEC esta na proximidade com o cliente: atendimento direto, entendimento do processo e suporte para compras, engenharia e operacao decidirem com seguranca.</p><div><strong>100+</strong><small>usinas atendidas</small></div><a className="btn primary" href="/contato">Falar com a IMEC <ChevronRight size={16} /></a></div><div className="clients-grid">{clientCards.map(([title, text], index) => <article key={title}><Users /><small>{String(index + 1).padStart(2, '0')}</small><h3>{title}</h3><p>{text}</p></article>)}</div></section><section className="client-journey"><div><span>Como atendemos clientes</span><h2>Metodo claro para transformar necessidade em entrega tecnica</h2><p>Da primeira conversa ao acompanhamento em campo, o processo ajuda engenharia, compras e operacao a decidirem com mais seguranca.</p></div><div>{attendanceSteps.map(([Icon, title, text], index) => <article key={title}><small>{String(index + 1).padStart(2, '0')}</small><Icon /><b>{title}</b><span>{text}</span></article>)}</div></section><section className="client-process"><article><FileText /><b>Escopo claro</b><span>Entendimento tecnico e comercial antes da proposta.</span></article><article><Factory /><b>Execucao conectada</b><span>Fabricacao, montagem e campo trabalhando com o mesmo objetivo.</span></article><article><Gauge /><b>Performance acompanhada</b><span>Suporte depois da entrega para reduzir risco operacional.</span></article></section></>;
}
function WorkWithUs() {
