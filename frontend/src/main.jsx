...
function Clients() {
  const clientCards = [
    [Factory, 'Mais de 100 usinas de acucar e alcool atendidas', 'Experiencia solida em manutencao, montagem e fabricacao para o setor sucroenergetico.'],
    [Building2, 'Industria alimenticia', 'Solucoes tecnicas para processos industriais que exigem qualidade, seguranca e eficiencia.'],
    [Settings, 'Destilarias e plantas de etanol', 'Atuacao especializada do preparo da cana a destilacao, contemplando todas as etapas do processo.'],
    [Gauge, 'Projetos industriais com acompanhamento de performance', 'Gestao tecnica de projetos com foco em resultados, prazos e confiabilidade.']
  ];
  return <><section className="clients-hero"><div className="clients-hero-copy"><span className="section-kicker">Clientes</span><h1>Experiencia com mais de <strong>100</strong> usinas</h1><p>A IMEC construiu sua trajetoria atendendo usinas de acucar e alcool e empresas da industria alimenticia em projetos, equipamentos e servicos especializados.</p></div><div className="clients-hero-metrics"><article><Factory /><b>100+</b><span>Usinas atendidas</span></article><article><ShieldCheck /><span>Atuacao em</span><b>projetos, equipamentos e servicos</b></article><article><Users /><span>Relacionamento</span><b>tecnico proximo</b></article></div></section><section className="clients-premium compact-clients"><div className="clients-grid">{clientCards.map(([Icon, title, text], index) => <article key={title}><Icon /><small>{String(index + 1).padStart(2, '0')}</small><h3>{title}</h3><p>{text}</p><a className="client-card-link" href="/contato" aria-label={`Falar sobre ${title}`}><ChevronRight size={18} /></a></article>)}</div><aside className="clients-highlight"><Users /><h2>Relacionamento tecnico</h2><p>O diferencial da IMEC esta na proximidade com o cliente: comunicacao direta com o responsavel pelo projeto e acompanhamento de performance apos a entrega.</p><div><span><ShieldCheck size={15} /> Atendimento consultivo</span><span><ShieldCheck size={15} /> Processos transparentes</span><span><ShieldCheck size={15} /> Suporte pos-entrega</span></div><a className="btn primary" href="/contato">Falar com a IMEC <ChevronRight size={16} /></a></aside></section></>;
}
function WorkWithUs() {
