...
function Clients({ data = {} }) {
  const settings = officialSettings(data.settings || {});
  const clientCards = [
    [Factory, 'Usinas de acucar e alcool', 'Manutencao, montagem e fabricacao para operacoes criticas do setor sucroenergetico.'],
    [Building2, 'Industria alimenticia', 'Solucoes tecnicas para processos industriais que exigem qualidade, seguranca e eficiencia.'],
    [Settings, 'Destilarias e plantas de etanol', 'Apoio do preparo da cana a destilacao, com foco em disponibilidade e continuidade operacional.'],
    [Gauge, 'Projetos com performance acompanhada', 'Gestao tecnica com escopo claro, prazos realistas e acompanhamento depois da entrega.']
  ];
  const proof = [
    ['Desde', '1998', 'historia industrial'],
    ['Mais de', '100+', 'usinas atendidas'],
    ['Fornecedor', 'BNDES', 'cadastro ativo'],
    ['Atuacao', 'Nacional', 'suporte ao Brasil']
  ];
  const whatsapp = whatsappUrl(settings, 'Ola! Quero falar com a IMEC sobre atendimento para usinas, clientes e projetos industriais.');
  return <><section className="clients-hero"><div className="clients-hero-copy"><span className="section-kicker">Clientes</span><h1>Experiencia com mais de <strong>100</strong> usinas</h1><p>A IMEC construiu sua trajetoria atendendo usinas de acucar e alcool e empresas da industria alimenticia em projetos, equipamentos e servicos especializados.</p><div className="clients-hero-actions"><a className="btn primary" href={whatsapp} target="_blank" rel="noreferrer"><MessageCircle size={17} /> Falar com especialista</a><a className="btn outline" href="/produtos">Ver equipamentos <ChevronRight size={16} /></a></div></div><div className="clients-hero-metrics"><article><Factory /><b>100+</b><span>Usinas atendidas</span></article><article><ShieldCheck /><span>Atuacao em</span><b>projetos, equipamentos e servicos</b></article><article><Users /><span>Relacionamento</span><b>tecnico proximo</b></article></div></section><section className="clients-premium compact-clients"><div className="clients-grid">{clientCards.map(([Icon, title, text], index) => <article key={title}><Icon /><small>{String(index + 1).padStart(2, '0')}</small><h3>{title}</h3><p>{text}</p><a className="client-card-link" href={whatsapp} target="_blank" rel="noreferrer" aria-label={`Falar sobre ${title}`}><ChevronRight size={18} /></a></article>)}</div><aside className="clients-highlight"><Users /><h2>Relacionamento tecnico</h2><p>O diferencial da IMEC esta na proximidade com o cliente: comunicacao direta com o responsavel pelo projeto e acompanhamento de performance apos a entrega.</p><div><span><ShieldCheck size={15} /> Atendimento consultivo</span><span><ShieldCheck size={15} /> Processos transparentes</span><span><ShieldCheck size={15} /> Suporte pos-entrega</span></div><a className="btn primary" href={whatsapp} target="_blank" rel="noreferrer">Falar com a IMEC <ChevronRight size={16} /></a></aside></section><section className="clients-proof">{proof.map(([label, value, text]) => <article key={value}><small>{label}</small><b>{value}</b><span>{text}</span></article>)}</section><section className="clients-final-cta"><div><span>Atendimento para clientes industriais</span><h2>Sua operacao precisa de suporte tecnico confiavel?</h2><p>Fale com a IMEC para avaliar demanda, prazo, escopo e melhor caminho tecnico antes da parada ou compra.</p></div><a className="btn primary" href={whatsapp} target="_blank" rel="noreferrer"><MessageCircle size={17} /> Solicitar atendimento pelo WhatsApp</a></section></>;
}
function WorkWithUs() {
